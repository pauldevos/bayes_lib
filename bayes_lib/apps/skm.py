...
def gillespie_simulate_par(initial_state, reactions, rate_fx, times, end_T = None):
        ...
        while not terminate and cur_state[0] <= end_T:
            ...
            if times is not None:
                while not terminate and updated_state[0] > times[0]:
                    states.append(np.append(times[0], cur_state[1:]))
                    times = times[1:]
                    if len(times) == 0:
                        terminate = True
            else:
...
class StochasticKineticRandomVariable(RandomVariable):
    ...
    def log_density(self, n_smc = 50, v = None):
        ...
        for i in range(1,T):
            ...
            res = list(p.map(lambda x: gillespie_simulate_par(x, self.reactions, rate_fx, np.array([0,deltas[i-1]])), resampled_particles))
    ...
    def gillespie_simulate(self, initial_state, reactions, rate_fx, times = None, tau_fn = None):
        ...
        if tau_fn:
            ...
            return gillespie_simulate_par(initial_state, reactions, rate_fx, times, end_T = self.end_T)
